...
async def booru(triggerMessage):
    ...
    if (len(tokens) <= 2):
        ...
        return
    if (triggerMessage.channel.type is discord.ChannelType.text and triggerMessage.channel.is_nsfw()) not (triggerMessage.channel.type is discord.ChannelType.group or triggerMessage.channel.type is discord.ChannelType.text):
