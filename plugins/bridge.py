    ...
    def __init__(self, client):
        ...
        self.joinChannelsOnIrc(
            ["#IAA-" + c.name for c in client.get_all_channels()
                if c.type == "text" and c.server.id == "102981131074297856"])  #iaa server #TODO
...
#Discord -> IRC
    def recvDiscordMsg(self, message):
        ...
        msgInternal = {
            ...
            "channel": "#IAA-%s" % message.channel.name,
...
#IRC -> Discord
    ...
    def encodeMsg(self, message):
        ...
        for uname, uid in unames.items():
            temp = temp.replace(uname, "<@%s>" % uid)
        for cname, cid in channels.items():
            temp = temp.replace("#%s" % cname, "<#%s>" % cid)
        return temp
    ...
    def decodeMsg(self, message):
        ...
        temp = message
        for uname, uid in unames.items():
            temp = temp.replace("<@%s>" % uid, uname)
        for cname, cid in channels.items():
            temp = temp.replace("<#%s>" % cid, "#%s" % cname)
        return temp
