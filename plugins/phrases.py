...
class Phrases:
    ...
    def lart(self, message):
        ...
        print("Lart: " + lart)
        asyncio.ensure_future(self.client.send_message(message.channel, lart))
    ...
    def praise(self, message):
        ...
        print("praise: " + praise)
        asyncio.ensure_future(self.client.send_message(message.channel, praise))
    ...
    def baddragon(self, message):
        ...
        print("dragon: " + dragon)
        asyncio.ensure_future(self.client.send_message(message.channel, dragon))

    def pal(self, message):
        if random.randrange(100) == 0:
            asyncio.ensure_future(self.client.send_message(message.channel, "Go fuck yourself " + message.author.name))
        else:
            asyncio.ensure_future(self.client.send_message(message.channel, "Yes this is pal, yes this is 60 FPS"))

    def mango(self, message):
        asyncio.ensure_future(self.client.send_message(message.channel, "mang0 is in jail for multishining a kangaroo BibleThump"))

    def eightball(self, message):
        ball = self.phrasebank["8balls"][random.randrange(self.phrasebank["8balls"].__len__())]
        print("8ball: " + ball)
        asyncio.ensure_future(self.client.send_message(message.channel, ":8ball: " + ball))

    def tests(self, message):
        target = message.author
        asyncio.ensure_future(self.client.send_message(target, "Take each of the following tests. Save a screenshot of each result.\nMBTI: http://www.humanmetrics.com/cgi-win/jtypes2.asp\nEnneagram: http://www.eclecticenergies.com/enneagram/test.php Take both tests for best results.\nBIG5: http://personality-testing.info/tests/BIG5.php\nTemperaments: http://personality-testing.info/tests/O4TS/"))
    ...
    def popori(self, message):
        asyncio.ensure_future(self.client.send_message(message.channel, "https://www.youtube.com/watch?v=gbNN3grTdDk"))

    def constitution(self, message):
        asyncio.ensure_future(self.client.send_message(message.channel, "https://docs.google.com/document/d/1XKFRQhzxwhjjtW1RnCc_ZhesRqavvM3F5z1LALfve48/"))

    def wiki(self, message):
        asyncio.ensure_future(self.client.send_message(message.channel, "http://iaa.wikia.com"))

    def application(self, message):
        asyncio.ensure_future(self.client.send_message(message.channel, "https://docs.google.com/forms/d/1bURXLKIo30XLX3RASg2B8XVLsYpp6GYXn84IT2F9hbM/viewform"))

    def unflip(self, message):
        asyncio.ensure_future(self.client.send_message(message.channel, "┬─┬﻿ ノ( ゜-゜ノ)"))

    def floor(self, message):
        asyncio.ensure_future(self.client.send_message(message.channel, "http://i.imgur.com/3OYeOxK.jpg"))

    def sonic(self, message):
        if random.randint(0,1) != 0:
            asyncio.ensure_future(self.client.send_message(message.channel, "http://archiesonic.wikia.com/wiki/Special:Random"))
        else:
            asyncio.ensure_future(self.client.send_message(message.channel, "http://sonicfanon.wikia.com/wiki/Special:Random"))
    def plot(self, message):
        if self.client.user in message.mentions:
            asyncio.ensure_future(self.client.send_message(message.channel, self.phrasebank["plots"][random.randrange(self.phrasebank["plots"].__len__())]))
    def addphrase(self, message):
        ...
        if phrasebanktarget is not None and phrasebanktarget in self.phrasebank and phrase is not None and perms:
            ...
            with open('phrasebank.json', 'w') as phrasefile:
                ...
                asyncio.ensure_future(self.client.send_message(message.channel, "Added phrase " + phrase))
